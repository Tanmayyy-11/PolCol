...
function find_centre(shape)
{
    ...
    let centre = new THREE.Vector3(newx*3/numVertices1,newy*3/numVertices1,newz*3/numVertices1);
	return centre;	
}
...
{	
	let origin  =  new THREE.Vector3(0,0,0);
	return subvec(getFarthestPoint(shape1,direction),getFarthestPoint(shape2,subvec(origin,direction)));
}
...
function triangularsimplex(obj)
{
	...
	if(abperp.dot(ao)>0)			//origin is in region ab
	{
		simplex[0]=b;
		simplex[1]=a;
		d = abperp;
	...
	else if(acperp.dot(ao)>0)			//origin is in region ac
	{
		simplex[0]=c;
		simplex[1]=a;
		d = acperp;
...
function vectortripleprod(vec1,vec2,vec3)         //returns (vec1xvec2)xvec3
{
	vec1.cross(vec2);
	vec1.cross(vec3);
